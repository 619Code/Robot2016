...
/**
 * The IMUVeryAdvanced class provides a simplified interface to advanced capabilities
 * of the KauaiLabs nav6 IMU.
 * 
 * The IMUVeryAdvanced class enables access to basic connectivity and state information, 
 * as well as key orientation information (yaw, pitch, roll, compass heading).
 * Additionally, the IMUVeryAdvanced class also provides access to extended information
 * including linear acceleration, motion detection, and sensor temperature.
 * @author Scott
 */public class IMUVeryAdvanced extends IMU {
    ...
    volatile float world_linear_acceleration_avg_x;
    volatile float world_linear_acceleration_avg_y;
    volatile float temp_c;
    double init_time;
    float world_linear_accel_history[];
    int   next_world_linear_accel_history_index;
    int   next_world_linear_accel_history_index_xy;
    float world_linear_acceleration_recent_avg;

    static final int WORLD_LINEAR_ACCEL_HISTORY_LENGTH = 10;

    /**
     * Constructs the IMUVeryAdvanced class, overriding the default update rate
     ...
     */
    public IMUVeryAdvanced(SerialPort serial_port, byte update_rate_hz) {
    ...
    /**
     * Constructs the IMUVeryAdvanced class, using the default update rate.  
     * 
     * Note that calculation of some advanced values utilizes additional 
     * cpu cycles, when compared to the IMU class.
     * @param serial_port BufferingSerialPort object to use
     */
    public IMUVeryAdvanced(SerialPort serial_port) {
    ...
    /**
     * Returns the average linear acceleration in the x-axis (in g).
     * 
     * World linear acceleration refers to raw acceleration data, which
     * has had the gravity component removed, and which has been rotated to
     * the same reference frame as the current yaw value.  The resulting
     ...
     * body (e.g., the robot) on which the nav6 IMU is mounted.
     * 
     * @return Average world linear acceleration in the x-axis (in g).
     */
    public float getAverageWorldAccelX()
    {
    	return world_linear_acceleration_avg_x;
    }

    /**
     * Returns the average linear acceleration in the y-axis (in g).
     * 
     * World linear acceleration refers to raw acceleration data, which
     * has had the gravity component removed, and which has been rotated to
     * the same reference frame as the current yaw value.  The resulting
     ...
     * body (e.g., the robot) on which the nav6 IMU is mounted.
     * 
     * @return Average world linear acceleration in the y-axis (in g).
     */
    public float getAverageWorldAccelY()
    {
    	return world_linear_acceleration_avg_y;
    }
    ...
    /**
     * Returns the system clock time (in milliseconds) at initialization
     * 
     * This value may be used for calculating velocity and position based
     * on the average acceleration and the change in time
    ...
    private void initWorldLinearAccelHistory(){
        ...
        next_world_linear_accel_history_index_xy = 0;
        world_linear_acceleration_avg_x = (float) 0.0;
        world_linear_acceleration_avg_y = (float) 0.0;
    ...
    private void updateWorldLinearAccelHistory( float x, float y, float z ){
    	...
    	world_linear_acceleration_avg_x += x;
    	world_linear_acceleration_avg_y += y;
    	next_world_linear_accel_history_index_xy++;
    	world_linear_acceleration_avg_x /= next_world_linear_accel_history_index_xy;
    	world_linear_acceleration_avg_y /= next_world_linear_accel_history_index_xy;
